...
export function ArtistCard({ artist }) {
  return (
    <article className="group overflow-hidden rounded-2xl border border-[#51AFF7]/25 bg-white shadow-lg shadow-black/10 transition duration-300 hover:-translate-y-1 hover:border-[#51AFF7]">
      <img
        src={artist.imagem}
      ...
      <div className="space-y-4 p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <h2 className="text-2xl font-extrabold text-[#0B1E30]">{artist.nome}</h2>
          </div>
          <FiMusic className="shrink-0 text-[#FFD900]" size={22} />
        </div>
        <p className="text-sm leading-6 text-slate-600">{artist.descricao}</p>
